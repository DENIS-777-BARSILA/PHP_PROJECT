...
@section('content')
    <form action="{{ url('/admin/rides/' . $ride->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="form-group">
            ...
            <input type="number" name="driver_id" value="{{ old('driver_id', $ride->driver_id) }}" required>
        </div>
        
        <div class="form-group">
            <label>Машина (ID)</label>
            <input type="number" name="car_id" value="{{ old('car_id', $ride->car_id) }}" required>
        </div>
